...
class UpdateInternshipPage extends StatefulWidget {
  final GetInternshipRes internship;
  const UpdateInternshipPage({super.key, required this.internship});

  @override
  State<UpdateInternshipPage> createState() => _UpdateInternshipPageState();
}

class _UpdateInternshipPageState extends State<UpdateInternshipPage> {
  TextEditingController titleCtrl = TextEditingController();
  TextEditingController locationCtrl = TextEditingController();
  TextEditingController companyCtrl = TextEditingController();
  TextEditingController emailCtrl = TextEditingController();
  TextEditingController descriptionCtrl = TextEditingController();
  TextEditingController stipentCtrl = TextEditingController();
  ...
  void initState() {
    super.initState();
    titleCtrl = TextEditingController(text: widget.internship.title);
    locationCtrl = TextEditingController(text: widget.internship.location);
    companyCtrl = TextEditingController(text: widget.internship.company);
    emailCtrl = TextEditingController(text: widget.internship.email);
    descriptionCtrl =
        TextEditingController(text: widget.internship.description);
    stipentCtrl = TextEditingController(text: widget.internship.stipend);
    periodCtrl = TextEditingController(text: widget.internship.period);
    contractCtrl = TextEditingController(text: widget.internship.contract);

    requirements = widget.internship.requirements;
    skillsRequired = widget.internship.skillsRequired;
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Consumer<MyInternshipNotifier>(
          builder: (context, myInternshipNotifier, child) {
        return SafeArea(
            child: SingleChildScrollView(
          child: Padding(
            padding: EdgeInsets.symmetric(horizontal: 10.w, vertical: 10.h),
            child: Form(
              key: myInternshipNotifier.formKey,
              child: Column(
                children: [
                  ...
                  const HeightSpacer(size: 10),
                  const HeadingWidget(text: 'Internship Details'),
                  const HeightSpacer(size: 10),
                  CustomTextFormField(
                  ...
                  CustomButton(
                    onTap: () async {
                      if (!myInternshipNotifier.validAndSave()) {
                        Get.snackbar(
                          'Updation Failed',
                      ...
                      } else if (myInternshipNotifier.imageFil.isEmpty &&
                          myInternshipNotifier.imageUrl == null) {
                        Get.snackbar(
                        ...
                        myInternshipNotifier.updateInternship(
                            model: InternshipReqModel(
                                email: emailCtrl.text,
                                title: titleCtrl.text,
                                location: locationCtrl.text,
                                company: companyCtrl.text,
                                description: descriptionCtrl.text,
                                stipend: stipentCtrl.text,
                                contract: contractCtrl.text,
                                duration: durationCtrl.text,
                                period: periodCtrl.text,
                                imageUrl: myInternshipNotifier.imageUrl!,
                                agentId: agentId,
